...
case class Thing(expr: Expr, isEscaped: Boolean) extends Renderable {
  def render(context: Context, options: Map[String, Any])(implicit sb: Scalabars) = {
    ...
    if (isEscaped && !options.get(noEscape.toString).asInstanceOf[Option[Boolean]].getOrElse(false) && sb.eval("this.safestring") == false)
