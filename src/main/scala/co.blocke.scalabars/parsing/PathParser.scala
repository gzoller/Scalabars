...
case class PathParser() {
  ...
  private def path[_: P] =
    P(element ~~ (separator ~~ element).repX)
      .map {
        case (f, pair) => (f + pair.map(s => s._1 + s._2).mkString(""), f +: pair.map(_._2).toList)
      }
  private def element[_: P] = P(specialVar | indexElement | normalElement)
  private def indexElement[_: P] =
    P(CharsWhileIn("""0-9""").repX(1) | "[" ~~ CharsWhileIn("""0-9""").repX(1) ~~ "]").!
  private def specialVar[_: P] =
  ...
  def LabelWithSpaces(implicit ctx: P[_]): P[String] = {
    ...
    while (!done && index < input.length) input(index) match {
      case LabelSpaces(x) => index += 1
      case _              => done = true
    }
    // Not sure this is needed.  @root determination should be made in Context
    //    if (index == start)
