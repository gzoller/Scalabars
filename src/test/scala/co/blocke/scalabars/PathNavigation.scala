    ...
    "-------------------------------\n:  Path Navigation (Context)  :\n-------------------------------") {
      describe("Lookup") {
        ...
        it("Simple element lookup") {
          root.lookup("/interests").show should be(
        ...
        it("Backgrack lookup (from root node)") {
          root.lookup("/interests/../ok").show should be(
            """/ok --> JBool(true)...
            |   Stack:
            |      / --> JObject(List((name,JString(Greg)), (age,JInt(53)), (ok,JBool(true)), (inter...
            |   Data: Map()
        ...
        it("Path item not found") {
          root.lookup("bogus").show should be(
            """/bogus --> JNothing...
            |   Stack:
            |      / --> JObject(List((name,JString(Greg)), (age,JInt(53)), (ok,JBool(true)), (inter...
            |   Data: Map()
      ...
      describe("Render") {
        ...
        it("Path item not found (strict)") {
          the[BarsException] thrownBy sb.compile("{{bogus}}", Map("strict" -> true))(json) should have message "Path not found: /bogus"
        }
      }
    }
}
